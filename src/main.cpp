...
void drawArmor(const vector<Armor> &armors, Mat &frame)
{
    for (const auto &armor : armors)
    {
        ...
        {
            intPts.push_back(Point(cvRound(pt.x), cvRound(pt.y))); // 四舍五入到最近整数
        }
...
int main()
{
    ...
    TargetSelector targetSelector(Config::get());
    ...
    while (true)
    {
        ...
        if (tracker.isInitialized())
        {
            ...
            if (aim.yaw < -100.0 || aim.yaw > 50.0)
                continue;
            if (aim.pitch < -40.0 || aim.pitch > 20.0)
                continue;
        }
        ...
        if (hasTarget)
        {
            ...
            string angleText = format("Yaw: %.2f (origin) / %.2f (kalman) / %.2f (predict)", pnpRes.yaw, filteredYaw, predictedYaw); //
